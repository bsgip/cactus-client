...
class RouteBehaviour:
    ...
    def xml(status: HTTPStatus, file_name: str) -> "RouteBehaviour":
        with open("tests/data/" + file_name) as fp:
            raw_xml = fp.read()
        return RouteBehaviour(
            status, raw_xml.encode(), {"Content-Type": MIME_TYPE_SEP2}
        )
...
async def create_test_session(
    aiohttp_client, routes: list[TestingAppRoute]
...
@pytest.mark.parametrize(
    ...
    product(
        [HTTPStatus.NOT_FOUND, HTTPStatus.UNAUTHORIZED, HTTPStatus.FORBIDDEN], [0, 2000]
    ),
)
@pytest.mark.asyncio
async def test_delete_and_check_resource_for_step_success(
    ...
    delete_route = TestingAppRoute(
        HTTPMethod.DELETE, "/foo/bar", [RouteBehaviour(HTTPStatus.OK, b"", {})]
    )
    get_route = TestingAppRoute(
        HTTPMethod.GET, "/foo/bar", [RouteBehaviour(refetch_status, b"", {})]
    )
    async with create_test_session(
        aiohttp_client, [delete_route, get_route]
    ) as session:
        execution_context, step_execution = testing_contexts_factory(session)
        execution_context.server_config = replace(
            execution_context.server_config, refetch_delay_ms=refetch_delay
        )

        start = datetime.now()
        await delete_and_check_resource_for_step(
            step_execution, execution_context, "/foo/bar"
        )
        finish = datetime.now()
    ...
    if refetch_delay == 0:
        assert (
            finish - start
        ).total_seconds() < 1, "There shouldn't have been any delay"
    else:
...
async def test_delete_and_check_resource_for_step_delete_bad_response(
    ...
    delete_route = TestingAppRoute(
        HTTPMethod.DELETE, "/foo/bar", [RouteBehaviour(delete_status, b"", {})]
    )
    get_route = TestingAppRoute(
        HTTPMethod.GET, "/foo/bar", [RouteBehaviour(HTTPStatus.NOT_FOUND, b"", {})]
    )
    async with create_test_session(
        aiohttp_client, [delete_route, get_route]
    ) as session:
...
async def test_get_resource_for_step_success(aiohttp_client, testing_contexts_factory):
    ...
    async with create_test_session(
        ...
        execution_context, step_execution = testing_contexts_factory(session)
        result = await get_resource_for_step(
...
@pytest.mark.asyncio
async def test_get_resource_for_step_bad_request(
...
@pytest.mark.asyncio
async def test_get_resource_for_step_xml_failure(
    aiohttp_client, testing_contexts_factory
):
...
@pytest.mark.parametrize(
    "has_property_changes, refetch_delay", product([True, False], [0, 2000])
)
...
async def test_submit_and_refetch_resource_for_step_success_no_location_header(
    ...
    async with create_test_session(
        ...
        [
            TestingAppRoute(
                HTTPMethod.PUT, "/foo", [RouteBehaviour(HTTPStatus.NO_CONTENT, b"", {})]
            ),
            TestingAppRoute(
                HTTPMethod.GET, "/foo", [RouteBehaviour.xml(HTTPStatus.OK, "dcap.xml")]
            ),
        ],
    ) as session:
        execution_context, step_execution = testing_contexts_factory(session)
...
@pytest.mark.asyncio
async def test_submit_and_refetch_resource_for_step_failure_initial_request(
    aiohttp_client, testing_contexts_factory
):
...
@pytest.mark.asyncio
async def test_submit_and_refetch_resource_for_step_failure_refetch_request(
    aiohttp_client, testing_contexts_factory
):
...
async def test_paginate_list_resource_items(aiohttp_client, testing_contexts_factory):
    ...
    assert len(execution_context.warnings.warnings) == 0
    assert (
        len(execution_context.responses.responses) == 3
    ), "we requested 3 pages of data"
...
@pytest.mark.asyncio
async def test_paginate_list_resource_items_handle_failure(
    aiohttp_client, testing_contexts_factory
):
    ...
    async with create_test_session(
        ...
        [
            TestingAppRoute(
                ...
                [
                    RouteBehaviour.xml(HTTPStatus.OK, "edev-list-1.xml"),
                    RouteBehaviour.xml(
                        HTTPStatus.INTERNAL_SERVER_ERROR, "edev-list-2.xml"
                    ),
                    RouteBehaviour.xml(
                        HTTPStatus.OK, "edev-list-empty.xml"
                    ),  # Should never run
                ],
            )
        ],
        ...
        with pytest.raises(RequestError):
            await paginate_list_resource_items(
                ...
                "/foo/bar",
                2,
                lambda list_response: cast(
                    EndDeviceListResponse, list_response
                ).EndDevice,
            )
    ...
    assert len(execution_context.warnings.warnings) == 0
    assert (
        len(execution_context.responses.responses) == 2
    ), "we requested 2 pages of data (we aborted due to failure)"
    assert "?s=0&l=2" in execution_context.responses.responses[0].url
    assert "?s=2&l=2" in execution_context.responses.responses[1].url


@pytest.mark.asyncio
async def test_paginate_list_resource_items_bad_all_count(
    aiohttp_client, testing_contexts_factory
):
    ...
    # Assert - contents of trackers
    assert (
        len(execution_context.warnings.warnings) == 1
    ), "The all count said 3 - but we only got 2"
    assert (
        len(execution_context.responses.responses) == 2
    ), "we requested 2 pages of data"
    assert "?s=0&l=2" in execution_context.responses.responses[0].url
    assert "?s=2&l=2" in execution_context.responses.responses[1].url


@pytest.mark.asyncio
async def test_paginate_list_resource_items_empty_list(
    aiohttp_client, testing_contexts_factory
):
    """Does paginate_list_resource_items work with an empty list"""
    behaviour = RouteBehaviour.xml(HTTPStatus.OK, "edev-list-empty.xml")
    behaviour.body = (
        behaviour.body.decode().replace('all="3"', 'all="0"').encode()
    )  # Make this a proper empty list
    async with create_test_session(
        aiohttp_client,
    ...
    assert len(execution_context.warnings.warnings) == 0
    assert (
        len(execution_context.responses.responses) == 1
    ), "we requested 1 page of data"
    assert "?s=0&l=3" in execution_context.responses.responses[0].url


@pytest.mark.asyncio
async def test_paginate_list_resource_items_too_many_requests(
    aiohttp_client, testing_contexts_factory
):
    """Does paginate_list_resource_items handle failures in one of the pagination requests"""
    async with create_test_session(
    ...
    assert (
        len(execution_context.responses.responses) == 2
    ), "we requested 2 pages of data (we aborted early)"
    assert "?s=0&l=2" in execution_context.responses.responses[0].url
    assert "?s=2&l=2" in execution_context.responses.responses[1].url
...
def test_resource_to_sep2_xml():
    ...
    xml1 = resource_to_sep2_xml(
        generate_class_instance(EndDeviceRequest, seed=1, generate_relationships=True)
    )
    xml2 = resource_to_sep2_xml(
        generate_class_instance(EndDeviceRequest, seed=2, generate_relationships=True)
    )
    xml3 = resource_to_sep2_xml(
        generate_class_instance(
            EndDeviceRequest, seed=2, generate_relationships=True, optional_is_none=True
        )
    )
...
async def test_fetch_list_page(aiohttp_client, testing_contexts_factory):
    ...
    assert (
        f"?s={start}&l={limit}" in requested_url
    ), f"Expected params s={start}&l={limit} in URL"


@mock.patch("cactus_client.action.server.asyncio.sleep")
@pytest.mark.asyncio
async def test_request_for_step_429_retry_success(
    mock_sleep, aiohttp_client, testing_contexts_factory
):
    async with create_test_session(
        aiohttp_client,
        [
        ...
        response = await request_for_step(
            step_execution, execution_context, "/foo/bar", HTTPMethod.GET
        )
...
@mock.patch("cactus_client.action.server.asyncio.sleep")
@pytest.mark.asyncio
async def test_request_for_step_429_all_retries_exhausted(
    mock_sleep, aiohttp_client, testing_contexts_factory
):
    async with create_test_session(
        aiohttp_client,
    ...
    assert (
        len(execution_context.responses.responses) == len(RATE_LIMIT_RETRY_DELAYS) + 1
    )
...
@pytest.mark.asyncio
async def test_client_error_request_for_step_success(
    aiohttp_client, testing_contexts_factory
):
...
@pytest.mark.parametrize(
    "status_code", [HTTPStatus.OK, HTTPStatus.INTERNAL_SERVER_ERROR]
)
@pytest.mark.asyncio
async def test_client_error_request_for_step_non_client_error(
    ...
    async with create_test_session(
        ...
        with pytest.raises(RequestError):
            await client_error_request_for_step(
                step_execution, execution_context, "/foo/bar", HTTPMethod.POST, "body"
            )
...
@pytest.mark.asyncio
async def test_client_error_or_empty_list_request_for_step_success_error(
    aiohttp_client, testing_contexts_factory
):
...
@pytest.mark.parametrize(
    "status_code", [HTTPStatus.BAD_GATEWAY, HTTPStatus.INTERNAL_SERVER_ERROR]
)
@pytest.mark.asyncio
