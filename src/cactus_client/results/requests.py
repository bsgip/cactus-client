...
def generate_request_file(
    method: str, url: str, host: str | None, headers: dict[str, str] | CIMultiDict, body: str
) -> list[str]:
...
def generate_response_file(status: int, headers: dict[str, str] | CIMultiDict, body: str) -> list[str]:
...
def persist_server_response(base_dir: Path, idx: int, host: str, response: ServerResponse):
...
def persist_notification(base_dir: Path, idx: int, webhook_endpoint: str | None, notification: NotificationRequest):
    ...
    for client in context.clients_by_alias.values():
        if client.notifications:
            for sub_id, endpoint in client.notifications.endpoint_by_sub_alias.items():
                webhook_by_sub_id[sub_id] = endpoint.fully_qualified_endpoint
